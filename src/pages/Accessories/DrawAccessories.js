...
function DrawAccessories(){
    ...
    useEffect(()=>{
        fetch(`${SUPABASE_URL}/rest/v1/products?select=*&category_id=eq.6e84ca47-0dd2-46ba-a4e5-9138ed9de9d6`, {
            method: "GET",
            headers: {
        ...
        <div className="product">
            ...
            {dataProduct && dataProduct.map(item=>(
                <Col xxl={6} xl={6} lg={8} md={12} sm={12} xs={12} className="product__item" key={item.id}>
                    <NavLink to={`/accessories/${item.id}`}>
...
export default DrawAccessories;